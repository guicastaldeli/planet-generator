...
// include: C:\Users\casta\AppData\Local\Temp\tmpsmd2cd3s.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'out/a.out.data';
...
Module['FS_createPath']("/", "_data", true, true);
    ...
    for (var file of metadata['files']) {
      ...
      async function processPackageData(arrayBuffer) {
        ...
        Module['removeRunDependency'](`fp ${name}`);
          ...
          Module['removeRunDependency']('datafile_out/a.out.data');
      }
      Module['addRunDependency']('datafile_out/a.out.data');
    ...
    loadPackage({"files": [{"filename": "/_data/data_parser.cpp", "start": 0, "end": 20035}, {"filename": "/_data/data_parser.h", "start": 20035, "end": 24753}, {"filename": "/_data/default_planet_data.json", "start": 24753, "end": 25304}, {"filename": "/_data/default_preset.json", "start": 25304, "end": 31377}, {"filename": "/_data/options.json", "start": 31377, "end": 35133}, {"filename": "/_data/positions.json", "start": 35133, "end": 36389}], "remote_package_size": 36389});

  })();

// end include: C:\Users\casta\AppData\Local\Temp\tmpsmd2cd3s.js
// include: C:\Users\casta\AppData\Local\Temp\tmpfc89kl1v.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: C:\Users\casta\AppData\Local\Temp\tmpfc89kl1v.js
// include: C:\Users\casta\AppData\Local\Temp\tmpy11jxf80.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: C:\Users\casta\AppData\Local\Temp\tmpy11jxf80.js
...
var ASM_CONSTS = {
  10521128: ($0) => { try { const htmlString = UTF8ToString($0); const container = document.createElement('div'); container.innerHTML = htmlString; const controlsContainer = container.querySelector('.controls--container'); if(controlsContainer) { document.body.appendChild(controlsContainer); console.log('Controls appended to DOM from C++'); } } catch(err) { console.error('Error appending controls:', err); } },  
 10521521: () => { (function() { if(typeof window._showGenerator !== 'undefined') { console.log('onCustomPreset call'); window._showGenerator(); } else { console.error('showGenerator not found!'); } })(); },  
 10521711: () => { if(typeof window.onSave === 'function') { window.onSave() } },  
 10521775: ($0) => { const htmlString = UTF8ToString($0); const container = document.createElement('div'); container.innerHTML = htmlString; const generatorContainer = container.querySelector('#planet-creator-modal'); if(generatorContainer) { document.body.appendChild(generatorContainer); console.log('Generator container appended to DOM from C++'); } },  
 10522111: () => { (function() { const container = document.querySelector('#planet-creator-modal'); if(container) { container.style.display = 'block'; if(window.generatorController) { window.generatorController.showGenerator(); } const controlList = document.querySelector('.controls--container'); if(controlList) { controlList.style.display = 'none'; } } else { console.error('Generator err'); } })(); },  
 10522499: () => { const container = document.querySelector('#planet-creator-modal'); if(container) { container.style.display = 'none'; } const controlList = document.querySelector('.controls--container'); if(controlList) { controlList.style.display = 'flex'; } },  
 10522746: () => { if(window.pendingTextureData) { var json = JSON.stringify(window.pendingTextureData); var lengthBytes = lengthBytesUTF8(json) + 1; var strPtr = _malloc(lengthBytes); stringToUTF8(json, strPtr, lengthBytes); window.pendingTextureData = null; return strPtr; } return 0; },  
 10523018: () => { const controlList = document.querySelector('.controls--container'); if(controlList) { controlList.style.display = 'flex'; } },  
 10523146: ($0, $1, $2, $3) => { var nameStr = UTF8ToString($0); var dataStr = UTF8ToString($1); var w = $2; var h = $3; var obj = {}; obj['name'] = nameStr; obj['data'] = dataStr; obj['width'] = w; obj['height'] = h; window.pendingTextureData = obj; },  
 10523368: ($0) => { const htmlString = UTF8ToString($0); const container = document.createElement('div'); container.innerHTML = htmlString; const infoContainer = container.querySelector('.info--container'); if(infoContainer) { document.body.appendChild(infoContainer); } },  
 10523623: ($0, $1) => { const planetName = UTF8ToString($0); const planetInfo = UTF8ToString($1); if(typeof window.display === 'function') { window.display(planetName, planetInfo); } else { console.error('window display function not found'); } },  
 10523847: () => { const container = document.querySelector('.info--container'); if(container) { container.style.display = 'none'; } },  
 10523965: () => { const container = document.querySelector('.info--container'); if(container) { container.style.display = 'none'; } },  
 10524083: ($0, $1) => { try { const data = UTF8ToString($0); const fileName = UTF8ToString($1); const blob = new Blob( [data], { type: 'application/json' } ); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = fileName; document.body.appendChild(a); a.click(); setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }); console.log('Preset exported!', fileName); return 1; } catch(err) { console.error('Error exporting preset!', err); return 0; } },  
 10524584: () => { const input = document.createElement('input'); input.type = 'file'; input.accept = '.json,application/json'; input.style.display = 'none'; input.addEventListener('change', function(ev) { const file = ev.target.files[0]; if(!file) return; const reader = new FileReader(); reader.onload = function(e) { try { const content = e.target.result; console.log('file loaded!'); const contentLength = lengthBytesUTF8(content) + 1; const dataPtr = _malloc(contentLength); stringToUTF8(content, dataPtr, contentLength); Module._onFileImported(dataPtr); _free(dataPtr); } catch(err) { console.error('Error reading file:', err); } }; reader.onerror = function(e) { console.error('File reader err!', e); }; reader.readAsText(file); }); document.body.appendChild(input); input.click(); setTimeout(() => { document.body.removeChild(input); }, 100); },  
 10525420: ($0, $1) => { try { const key = UTF8ToString($0); const dataStr = UTF8ToString($1); localStorage.setItem(key, dataStr); console.log('Preset saved to localStorage with key:', key); return 1; } catch(err) { console.error('Error saving to local storage', err); return 0; } },  
 10525680: ($0) => { try { const key = UTF8ToString($0); const data = localStorage.getItem(key); if(data) { console.log('Found preset in localstorage with key: ', key); var lengthBytes = lengthBytesUTF8(data) + 1; var strPtr = _malloc(lengthBytes); stringToUTF8(data, strPtr, lengthBytes); return strPtr; } else { console.log('No preset found in local storage!: ', key); return 0; } } catch(err) { console.error('Error loading!', err); return 0. } },  
 10526111: ($0) => { try { const key = UTF8ToString($0); return localStorage.getItem(key) !== null ? 1 : 0; } catch(err) { console.error('Error checking localStorage', err); return 0; } },  
 10526280: ($0) => { try { const key = UTF8ToString($0); localStorage.removeItem(key); console.log('Removed preset from localStorage with key:', key); } catch(err) { console.error('Error clearing localStorage:', err); } },  
 10526483: ($0, $1) => { const canvas = document.getElementById("ctx"); if(canvas) { const dpr = window.devicePixelRatio || 1; const displayWidth = Math.max(1, Math.floor(canvas.clientWidth * dpr)); const displayHeight = Math.max(1, Math.floor(canvas.clientHeight * dpr)); canvas.width = displayWidth; canvas.height = displayHeight; setValue($0, displayWidth, 'i32'); setValue($1, displayHeight, 'i32'); console.log("HD Resize: " + displayWidth + " x " + displayHeight); } }
};
function getDevicePixelRatio() { return window.devicePixelRatio || 1; }
function setupCanvas(width,height,dpr) { const canvas = document.getElementById("ctx"); Module.canvas = canvas; const pixelRatio = window.devicePixelRatio || 1; const displayWidth = Math.floor(canvas.clientWidth * pixelRatio); const displayHeight = Math.floor(canvas.clientHeight * pixelRatio); canvas.width = displayWidth; canvas.height = displayHeight; HEAP32[width >> 2] = displayWidth; HEAP32[height >> 2] = displayHeight; HEAPF32[dpr >> 2] = pixelRatio; }

// Imports from the Wasm binary.
var _display = Module['_display'] = makeInvalidEarlyAccess('_display');
var _appendToDOM = Module['_appendToDOM'] = makeInvalidEarlyAccess('_appendToDOM');
var _onControlsMenu = Module['_onControlsMenu'] = makeInvalidEarlyAccess('_onControlsMenu');
var _onCustomPreset = Module['_onCustomPreset'] = makeInvalidEarlyAccess('_onCustomPreset');
var _onSavePreset = Module['_onSavePreset'] = makeInvalidEarlyAccess('_onSavePreset');
var _onImportPreset = Module['_onImportPreset'] = makeInvalidEarlyAccess('_onImportPreset');
var _onExportPreset = Module['_onExportPreset'] = makeInvalidEarlyAccess('_onExportPreset');
var _onResetToDefault = Module['_onResetToDefault'] = makeInvalidEarlyAccess('_onResetToDefault');
var _onClear = Module['_onClear'] = makeInvalidEarlyAccess('_onClear');
var _appendGeneratorToDOM = Module['_appendGeneratorToDOM'] = makeInvalidEarlyAccess('_appendGeneratorToDOM');
var _showGenerator = Module['_showGenerator'] = makeInvalidEarlyAccess('_showGenerator');
var _startGeneratorPreview = Module['_startGeneratorPreview'] = makeInvalidEarlyAccess('_startGeneratorPreview');
var _updatePreviewPlanet = Module['_updatePreviewPlanet'] = makeInvalidEarlyAccess('_updatePreviewPlanet');
var _cleanupPreview = Module['_cleanupPreview'] = makeInvalidEarlyAccess('_cleanupPreview');
var _clearCurrentTexture = Module['_clearCurrentTexture'] = makeInvalidEarlyAccess('_clearCurrentTexture');
var _hideGenerator = Module['_hideGenerator'] = makeInvalidEarlyAccess('_hideGenerator');
var _generatePlanetParser = Module['_generatePlanetParser'] = makeInvalidEarlyAccess('_generatePlanetParser');
var _free = makeInvalidEarlyAccess('_free');
var _getDefaultData = Module['_getDefaultData'] = makeInvalidEarlyAccess('_getDefaultData');
var _uploadTexture = Module['_uploadTexture'] = makeInvalidEarlyAccess('_uploadTexture');
var _appendInfoToDOM = Module['_appendInfoToDOM'] = makeInvalidEarlyAccess('_appendInfoToDOM');
var _closeMenu = Module['_closeMenu'] = makeInvalidEarlyAccess('_closeMenu');
var _deletePlanet = Module['_deletePlanet'] = makeInvalidEarlyAccess('_deletePlanet');
var _onFileImported = Module['_onFileImported'] = makeInvalidEarlyAccess('_onFileImported');
var _malloc = makeInvalidEarlyAccess('_malloc');
var _resizeCanvas = Module['_resizeCanvas'] = makeInvalidEarlyAccess('_resizeCanvas');
...
function assignWasmExports(wasmExports) {
  assert(typeof wasmExports['display'] != 'undefined', 'missing Wasm export: display');
  assert(typeof wasmExports['appendToDOM'] != 'undefined', 'missing Wasm export: appendToDOM');
  assert(typeof wasmExports['onControlsMenu'] != 'undefined', 'missing Wasm export: onControlsMenu');
  assert(typeof wasmExports['onCustomPreset'] != 'undefined', 'missing Wasm export: onCustomPreset');
  assert(typeof wasmExports['onSavePreset'] != 'undefined', 'missing Wasm export: onSavePreset');
  assert(typeof wasmExports['onImportPreset'] != 'undefined', 'missing Wasm export: onImportPreset');
  assert(typeof wasmExports['onExportPreset'] != 'undefined', 'missing Wasm export: onExportPreset');
  assert(typeof wasmExports['onResetToDefault'] != 'undefined', 'missing Wasm export: onResetToDefault');
  assert(typeof wasmExports['onClear'] != 'undefined', 'missing Wasm export: onClear');
  assert(typeof wasmExports['appendGeneratorToDOM'] != 'undefined', 'missing Wasm export: appendGeneratorToDOM');
  assert(typeof wasmExports['showGenerator'] != 'undefined', 'missing Wasm export: showGenerator');
  assert(typeof wasmExports['startGeneratorPreview'] != 'undefined', 'missing Wasm export: startGeneratorPreview');
  assert(typeof wasmExports['updatePreviewPlanet'] != 'undefined', 'missing Wasm export: updatePreviewPlanet');
  assert(typeof wasmExports['cleanupPreview'] != 'undefined', 'missing Wasm export: cleanupPreview');
  assert(typeof wasmExports['clearCurrentTexture'] != 'undefined', 'missing Wasm export: clearCurrentTexture');
  assert(typeof wasmExports['hideGenerator'] != 'undefined', 'missing Wasm export: hideGenerator');
  assert(typeof wasmExports['generatePlanetParser'] != 'undefined', 'missing Wasm export: generatePlanetParser');
  assert(typeof wasmExports['free'] != 'undefined', 'missing Wasm export: free');
  assert(typeof wasmExports['getDefaultData'] != 'undefined', 'missing Wasm export: getDefaultData');
  assert(typeof wasmExports['uploadTexture'] != 'undefined', 'missing Wasm export: uploadTexture');
  assert(typeof wasmExports['appendInfoToDOM'] != 'undefined', 'missing Wasm export: appendInfoToDOM');
  assert(typeof wasmExports['closeMenu'] != 'undefined', 'missing Wasm export: closeMenu');
  assert(typeof wasmExports['deletePlanet'] != 'undefined', 'missing Wasm export: deletePlanet');
  assert(typeof wasmExports['onFileImported'] != 'undefined', 'missing Wasm export: onFileImported');
  assert(typeof wasmExports['malloc'] != 'undefined', 'missing Wasm export: malloc');
  assert(typeof wasmExports['resizeCanvas'] != 'undefined', 'missing Wasm export: resizeCanvas');
  ...
  _display = Module['_display'] = createExportWrapper('display', 2);
  _appendToDOM = Module['_appendToDOM'] = createExportWrapper('appendToDOM', 1);
  _onControlsMenu = Module['_onControlsMenu'] = createExportWrapper('onControlsMenu', 0);
  _onCustomPreset = Module['_onCustomPreset'] = createExportWrapper('onCustomPreset', 0);
  _onSavePreset = Module['_onSavePreset'] = createExportWrapper('onSavePreset', 0);
  _onImportPreset = Module['_onImportPreset'] = createExportWrapper('onImportPreset', 1);
  _onExportPreset = Module['_onExportPreset'] = createExportWrapper('onExportPreset', 0);
  _onResetToDefault = Module['_onResetToDefault'] = createExportWrapper('onResetToDefault', 0);
  _onClear = Module['_onClear'] = createExportWrapper('onClear', 0);
  _appendGeneratorToDOM = Module['_appendGeneratorToDOM'] = createExportWrapper('appendGeneratorToDOM', 1);
  _showGenerator = Module['_showGenerator'] = createExportWrapper('showGenerator', 0);
  _startGeneratorPreview = Module['_startGeneratorPreview'] = createExportWrapper('startGeneratorPreview', 0);
  _updatePreviewPlanet = Module['_updatePreviewPlanet'] = createExportWrapper('updatePreviewPlanet', 1);
  _cleanupPreview = Module['_cleanupPreview'] = createExportWrapper('cleanupPreview', 0);
  _clearCurrentTexture = Module['_clearCurrentTexture'] = createExportWrapper('clearCurrentTexture', 0);
  _hideGenerator = Module['_hideGenerator'] = createExportWrapper('hideGenerator', 0);
  _generatePlanetParser = Module['_generatePlanetParser'] = createExportWrapper('generatePlanetParser', 1);
  _free = createExportWrapper('free', 1);
  _getDefaultData = Module['_getDefaultData'] = createExportWrapper('getDefaultData', 0);
  _uploadTexture = Module['_uploadTexture'] = createExportWrapper('uploadTexture', 4);
  _appendInfoToDOM = Module['_appendInfoToDOM'] = createExportWrapper('appendInfoToDOM', 1);
  _closeMenu = Module['_closeMenu'] = createExportWrapper('closeMenu', 0);
  _deletePlanet = Module['_deletePlanet'] = createExportWrapper('deletePlanet', 0);
  _onFileImported = Module['_onFileImported'] = createExportWrapper('onFileImported', 1);
  _malloc = createExportWrapper('malloc', 1);
  _resizeCanvas = Module['_resizeCanvas'] = createExportWrapper('resizeCanvas', 0);
